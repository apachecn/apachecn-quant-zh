...
NumPy 和 pandas 是自定义因子计算的关键工具。数据目录中的笔记本`00-data-prep.ipynb`包含了如何创建各种因子的例子。笔记本使用由 GitHub repo 根目录下的 data 文件夹中的`get_data.py`脚本生成的数据，并以`HDF5`格式存储，以便更快地访问。关于Pandas`DataFrames`的`parquet`、`HDF5`和`csv`存储格式的对比，请参见 GitHub repo 上[第二章](02.html)、*市场和基本面数据*目录中的笔记本`storage_benchmarks.ipynb`。
